    ...
    if add_selectbox == "数据预览":
        ...
        def my_pachong():
            with st.spinner('请稍等...'):
                ...
                # 读取数据文件
                df = pd.read_csv(
                    f'C:\\Users\yzhsd\PycharmProjects\pythonProject\实训项目\智联招聘\{cs}{job}智联招聘.csv')
                ...
                # 输出结果到新的csv文件
                df.to_csv(f'C:\\Users\yzhsd\PycharmProjects\pythonProject\实训项目\智联招聘\Clear{cs}{job}智联招聘.csv',
                          index=False)
                ...
                # 读取csv文件
                df1 = pd.read_csv(
                    f'C:\\Users\yzhsd\PycharmProjects\pythonProject\实训项目\智联招聘\Clear{cs}{job}智联招聘.csv',
                    encoding='utf-8')
        ...
        def 合并():
            with st.spinner('合并中...'):
                ...
                cursor = conn.cursor()
                # 选择需要合并的表格名称和目标表格名称
                table_names = ['上海司机数据', '南京司机数据', '重庆司机数据', '深圳司机数据', '北京司机数据',
                               '苏州司机数据']
                target_table_name = '全部司机数据'
                # 获取表格字段名称
                cursor.execute(
                    f"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='{table_names[0]}'")
        ...
        if uploaded_file is not None:
            ...
            if visualization_option == "折线图":
                ...
                gongsi = selected_df1.groupby('区').count()['工资']
                # 招聘公司所在地点
                pie1 = (
